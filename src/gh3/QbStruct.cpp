...
namespace GH3
{
    ...
    // Nearly exact translation from IDA. Ideally this should be cleaned up in the future.
    /// Retrieves a value from the specified key with the given value type and stores it in value.
	...
	inline void __declspec(naked) __stdcall NativeInsertCall(QbStruct *qbStruct, QbKey qbKey, T item)
	{
		...
		__asm
		{
			//Have to use [esp] because Visual Studio expects there to be a base pointer (ebp) if you use variable names
			mov ecx, [esp+4];
			push [esp+0Ch];
			push [esp+0Ch];
			call nativeFunction;
